...
def build_package(op_file):
    ...
    llvm_files = []
    for src in orcc_files:
        if not src.endswith(".orcat"):
            print(f"WARNING: ORCC source '{src}' does not end with .orcat (skipping).")
            continue
        out_ll = src[:-6] + ".ll"
        opt_out = src[:-6] + ".opt.ll"

        run(["./ORCC.bin", src, "-o", out_ll], check=True)
        run(["opt", "-O2", out_ll, "-o", opt_out], check=True)
        llvm_files.append(opt_out)
    ...
    link_cmd = ["clang", "-O2"] + llvm_files + obj_files + ldflags + manual_ld + ["-o", package_name]
    print("\nLink command:")
    print(" ".join(link_cmd))
