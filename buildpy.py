...
def build_package(op_file):
    ...
    if not orcc_files:
        ...
        out_ll = entry[:-6] + ".ll" if entry.endswith(".orcat") else entry + ".ll"
        opt_out = entry[:-6] + ".opt.ll" if entry.endswith(".orcat") else entry + ".opt.ll"

        run(["./ORCC.py", entry, "-o", out_ll], check=True)
